...
- 支持高风险场景下的监护人邮箱联动提醒
- 支持高风险结果确认门禁，用户需先确认已暂停进一步操作后再查看完整分析
- 适合演示真实用户场景中的统一分析能力
...
![](https://cdn.jsdelivr.net/gh/Song-xuan-xuan/PicGo_image@main/img/mermaid-diagram-2026-03-20-143603.png)
...
监护人邮箱通知服务由后端独立模块 `backend/app/services/guardian_notification_service.py` 负责，使用 `Resend` 官方 API 发送高风险预警邮件。

建议在 `backend/.env` 中配置：

```env
RESEND_API_KEY=your_real_resend_key
RESEND_FROM_EMAIL=你的的域名
RESEND_FROM_NAME=反诈智能助手
```

注意事项：

- `RESEND_FROM_EMAIL` 必须属于你在 `Resend` 中已验证通过的域名
- 修改 `backend/.env` 后需要重启后端，配置才会生效
- 若服务器部署使用现成数据库文件，请确保 `user_profiles` 表已包含：
  - `guardian_name`
  - `guardian_relation`
  - `guardian_email`
  - `guardian_notify_enabled`

### 监护人邮箱预警使用方式

1. 登录系统后，进入“个人信息 / 个人画像”页面
2. 填写监护人姓名、关系、邮箱，并开启“高风险时自动通知监护人”
3. 进入“多模态分析”页面，提交文本、截图或语音进行统一分析
4. 当系统判定为 `high` 风险且监护人邮箱已启用时，后端会自动尝试发送监护人提醒邮件
5. 前端会先显示高风险强提醒卡片，用户点击“我已暂停进一步操作”后，才会展示完整分析结果

当前版本中，监护人邮箱联动仅在多模态统一分析链路中触发，不会扩散到其他专项检测接口。
...
- 监护人邮箱未收到提醒：确认 `backend/.env` 中 `RESEND_API_KEY / RESEND_FROM_EMAIL / RESEND_FROM_NAME` 已填写，后端已重启，且个人画像中已经保存监护人邮箱并开启通知
- 页面出现高风险确认门禁但没有发信：优先检查 `/api/v1/agent/analyze` 返回中的 `guardian_notification.status` 与 `guardian_notification.message`
