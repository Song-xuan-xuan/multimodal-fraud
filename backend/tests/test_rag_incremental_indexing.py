...
class RagIncrementalIndexingTests(unittest.TestCase):
    def test_append_updates_manifest_and_inserts_new_documents(self) -> None:
        ...
        with patch.dict(sys.modules, fake_modules),              patch('app.services.rag_service.get_settings', return_value=fake_settings),              patch('model.common.device.resolve_device', return_value='cpu'),              patch('llama_index.embeddings.huggingface.HuggingFaceEmbedding', side_effect=lambda **kwargs: SimpleNamespace(**kwargs)):
