    ...
    <header class="dashboard-hero tech-panel">
      <div class="dashboard-hero__content">
        <div class="dashboard-hero__headline">
          ...
          <div class="dashboard-hero__briefs">
            <article v-for="item in commandBriefs" :key="item.label" class="dashboard-hero__brief-card">
        ...
        <div class="dashboard-hero__ledger">
          <div class="dashboard-hero__ledger-copy">
            <span>值守摘要</span>
            <strong>{{ formattedUpdatedAt }}</strong>
          </div>
          <p>重点地区、待复核线索与快捷入口已经归拢到同一张工作台卷宗，便于跨模块跟进。</p>
        </div>
...
.dashboard-hero__briefs {
  display: grid;
  grid-template-columns: 1fr;
  gap: 12px;
}

.dashboard-hero__brief-card,
.dashboard-command__brief,
.dashboard-hero__ledger {
  border: 1px solid color-mix(in srgb, var(--tech-color-primary) 16%, transparent);
...
.dashboard-hero__brief-card span,
.dashboard-command__brief span,
.dashboard-command__brief small,
.dashboard-hero__ledger-copy span,
.dashboard-hero__ledger p {
  color: var(--tech-text-secondary);
}

.dashboard-hero__brief-card strong,
.dashboard-command__brief strong,
.dashboard-hero__ledger-copy strong {
  color: var(--tech-text-primary);
}
