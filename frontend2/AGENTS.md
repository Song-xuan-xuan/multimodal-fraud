...
- 若在当前 PowerShell 环境下使用 `apply_patch` 创建或更新文件时出现 UTF-8 patch 参数错误，应立即停止重试。
- 此时优先直接调用底层 `codex.exe --codex-run-as-apply-patch` 执行补丁；如果目标文件尚不存在，则先请用户手动创建空文件，再继续编辑该现有文件。
- 一旦发生该情况，需将该约束写回本文件，避免后续回合重复踩坑。
...
你是一位精通 Vue.js 生态、拥有顶级 UI/UX 审美和深厚前端架构经验的资深前端工程师。

## 任务目标



我需要你对当前的 Vue 前端项目/组件进行一次彻底的“视觉大换血”（UI/UX 重构）。
项目的整体业务主题不变，但我希望看到更加现代化、美观、用户体验更好的界面设计。我已经做好了代码的完整备份，所以请尽情发挥你的设计创意，不用担心破坏现有视觉结构。

## 核心红线（绝对不可更改 `<script>` 中的业务核心）

为了保证项目重构后的正常运行，请你严格遵守以下原则：
1. 不修改网络请求：所有通过 axios/fetch 发起的 API 接口路径、请求方法、参数结构以及数据返回后的 `.then`/`await` 处理逻辑必须保持原样。
2. 不修改路由逻辑：保留所有的 `vue-router` 逻辑，包括 `router.push/replace`、携带的 params/query 参数、以及路由守卫。
3. 不修改状态管理：保留所有 Pinia 或 Vuex 的 `store` 引入、数据读取（state/getters）和修改（actions/mutations）逻辑。
4. 保留核心数据绑定：保留原有的响应式数据（`ref`、`reactive`），不要修改影响业务流的 `v-model` 绑定。保留原有的自定义 `props` 和 `emits` 事件触发逻辑。

## 允许修改的范围（尽情在 `<template>` 和 `<style>` 中发挥）

1. DOM 重构：大刀阔斧地重写 `<template>` 中的 HTML 结构。采用更现代的布局（如 Flexbox/CSS Grid），优化信息层级（如使用卡片式设计、侧边栏、合理的空白区）。
2. 视觉升级：全面升级 `<style>`（或 Tailwind 等原子类）。优化色彩搭配、排版、阴影、圆角等。
3. 交互与动画：在合适的地方利用 Vue 的 `<Transition>` 或 `<TransitionGroup>` 添加页面切换、元素出现的过渡动画。增加合理的悬浮状态（Hover）和加载骨架屏（Skeleton）。
4. UI 拆分：如果重构后的 `<template>` 过于臃肿，允许你在不改变核心逻辑的前提下，将纯 UI 展示部分拆分成新的 Vue 子组件（Dumb Components）。
